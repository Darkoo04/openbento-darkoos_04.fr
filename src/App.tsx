...
import { Youtube, Play, Loader2, X, ChevronLeft, ChevronRight } from 'lucide-react'
import {
...
const Lightbox = ({ images, startIndex, onClose }: LightboxProps) => {
  ...
  return (
    ...
    >
      {/* Close button */}
      <button
        className="absolute top-4 right-4 text-white bg-white/20 hover:bg-white/40 rounded-full w-10 h-10 flex items-center justify-center transition-all z-10"
        onClick={onClose}
        aria-label="Fermer"
      >
        <X size={20} />
      ...
      {images.length > 1 && (
        <button
          className="absolute left-4 text-white bg-white/20 hover:bg-white/40 rounded-full w-10 h-10 flex items-center justify-center transition-all z-10"
      ...
      <img
        src={images[current]}
        alt=""
        className="max-w-[88vw] max-h-[88vh] object-contain rounded-2xl shadow-2xl select-none"
      ...
      {images.length > 1 && (
        <button
          className="absolute right-4 text-white bg-white/20 hover:bg-white/40 rounded-full w-10 h-10 flex items-center justify-center transition-all z-10"
...
const Block = ({ block, onImageClick }: { block: BlockData; onImageClick?: (url: string) => void }) => {
  ...
  // Handler pour ouvrir la lightbox sur les blocs image (MEDIA sans lien externe)
  const handleImageClick = (e: React.MouseEvent) => {
    if (block.imageUrl && onImageClick) {
      e.stopPropagation()
      onImageClick(block.imageUrl)
    }
  }
  ...
  // Les blocs MEDIA s'ouvrent en lightbox, les blocs LINK redirigent vers le lien
  const isMediaOnly = block.type === BlockType.MEDIA && block.imageUrl
  ...
  return (
    <div
      onClick={isMediaOnly ? handleImageClick : handleClick}
      style={{ ...gridStyle }}
      className="cursor-pointer h-full transform-gpu"
    >
      <div ref={elementRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}
        ...
        {/* Icône loupe sur les blocs MEDIA pour indiquer qu'on peut agrandir */}
        {isMediaOnly && (
          <div className="absolute top-2 right-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 rounded-full p-1.5">
...
const allImageUrls = blocks
  .filter(b => b.type === BlockType.MEDIA && b.imageUrl)
...
export default function App() {
  ...
  const lightboxIndex = lightbox ? allImageUrls.indexOf(lightbox.url) : 0
  ...
  return (
    <div className="min-h-screen font-sans" style={bgStyle}>
      ...
      {lightbox && (
        <Lightbox
          images={allImageUrls.length > 0 ? allImageUrls : [lightbox.url]}
      ...
      <div className="relative z-10">
        ...
        <div className="hidden lg:flex">
          ...
          <div className="ml-[420px] flex-1 p-12">
            <div className="grid gap-2" style={{ gridTemplateColumns: 'repeat(9, 1fr)', gridAutoRows: '64px' }}>
              {blocks.map(block => <Block key={block.id} block={block} onImageClick={openLightbox} />)}
            </div>
          </div>
        </div>
        ...
        <div className="lg:hidden">
          ...
          <div className="p-4">
            <div className="grid" style={{ gridTemplateColumns: 'repeat(2, 1fr)', gridAutoRows: '90px', gap: '10px' }}>
              {sortedBlocks.map(block => {
                const mobile = getMobileLayout(block)
                return (
                  <div key={block.id} style={{ gridColumn: `span ${mobile.colSpan}`, gridRow: `span ${mobile.rowSpan}` }}>
                    <Block block={{ ...block, gridColumn: undefined, gridRow: undefined }} onImageClick={openLightbox} />
                  </div>
                )
              })}
            </div>
